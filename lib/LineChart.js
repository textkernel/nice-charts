...
class LineChart extends React.Component {
    ...
    drawAxisX(svg, isTimeScale) {
        ...
        const ticksCount = this.state.ticks.count;

        if (isTimeScale) {
            if (this.state.ticks.format) {
                axisBottom.tickFormat(
                    d3.timeFormat(this.state.ticks.format)
                );
            }
            if (ticksCount) {
                axisBottom.ticks(ticksCount);
            }
        }
    ...
    drawChart() {
        ...
        svg = d3.select(target).append('svg')
            ...
            .attr('class', 'nc-chart__svg nc-chart__line')
            .append('g')
    ...
    updateChart() {
        ...
        if (this.props.area) {
            let area = this.getArea(useTimeScale);
            svg.select('.nc-line__area')
                .data([data])
                .attr("d", area);
        }

        let dot = svg.selectAll('.nc-line__dot')
            .data(data);
