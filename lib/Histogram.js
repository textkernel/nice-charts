...
class Histogram extends React.Component {
    ...
    width(){
        return 600 - this.state.margin.left - this.state.margin.right;
    }

    height(){
        return 300 - this.state.margin.top - this.state.margin.bottom;
    }
    ...
    getTipRenderer(axes, step) {
        ...
        return d3tip()
            ...
            .html(d => {
                ...
                if (!_.isNil(this.state.ticks.prefix)) {
                    values.forEach((v, k) => {
                        values[k] = `${this.state.ticks.prefix}${v}`;
                    });
                }
    ...
    drawAxisX(svg) {
        ...
        svg.append('g')
            ...
            .selectAll('text')
                ...
                .attr('transform', () => {
                    let deg = -65;
                    if (!_.isNil(this.state.ticks.angle)) {
    ...
    updateChart() {
        let data = _.tail(this.state.data),
            step = data[1][0] - data[0][0],
            binWidth = parseFloat(this.width() / data.length) - 1,
            svg = this.state.svg;
