    ...
    drawChart(update) {
        ...
        if (!update) {
            tooltip = d3.select(target)
                .append('div')
                .attr('class', 'mi-tooltip');

            tooltip.append('div')
                .attr('class', 'mi-tooltip-label');

            tooltip.append('div')
                .attr('class', 'mi-tooltip-value');

            if (this.props.icon) {
                icon = d3.select(target)
                    .append('div')
                    .attr('class', 'mi-pie-icon-container');

                icon
                    .append('i')
                    .attr('class', _.join(['mi-pie-icon', 'fa', this.props.icon], ' '));
            }

            svg = d3
                .select(target)
                .append('svg')
                .attr('class', 'mi-chart-svg mi-chart-pie')
                .attr('width', '100%')
                .attr('height', '100%')
                .attr('viewBox',`0 0 ${this.width() + this.state.margin.left + this.state.margin.right + 10} ${this.height() + this.state.margin.top + this.state.margin.bottom}`)
                .attr('preserveAspectRatio','xMinYMin meet')
                .append('g')
                .attr('transform', `translate(${(this.width() + this.state.margin.left + this.state.margin.right) / 2}, ${(this.height() + this.state.margin.top + this.state.margin.bottom) / 2})`);
        } else {
            tooltip = d3.select(target).select('.mi-tooltip');
            icon = d3.select(target).select('.mi-pie-icon-container');
        }
        ...
        let newGroup = group.enter()
            .append('g')
            .attr('class', 'mi-pie-slice');

        let newSlice = newGroup
            .append('path')
            .attr('class', 'mi-pie-slice-path');
        ...
        const appendLabels = (newGroup, icon, tooltip) => {
            newGroup
                .append('text')
                .attr('transform', d => `translate(${arc.centroid(d)})`)
                .attr('dy', '.4em')
                .attr('text-anchor', 'middle')
                .attr('class', 'mi-pie-label inside')
                .text(d => {
                    let text = '';
                    if ((d.value / this.total()) * 100 > 5) {
                        text = this.format(d.value);
            ...
            newGroup
                .append('text')
                .attr('class', 'mi-pie-label outside')
                .text(d => {
                    let text = '';
                    if ((d.value / this.total()) * 100 > 5) {
                        text = utils.truncate(this._getLabel(d.data), {
        ...
        newGroup
            .on('mouseover', d => {
                ...
                tooltip.select('.mi-tooltip-label').html(this._getLabel(d.data));
                tooltip.select('.mi-tooltip-value').html(value);
                if (icon) {
                    icon.attr('class', 'mi-pie-icon-container mi-pie-icon-hidden');
                }
            ...
            .on('mouseout', () => {
                timer = setTimeout(() => {
                    if (icon) {
                        icon.attr('class', 'mi-pie-icon-container');
                    }
