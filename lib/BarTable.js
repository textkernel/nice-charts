    ...
    drawChart(update) {
        ...
        if (!update) {
            ...
            svg = d3.select(target).append('svg')
                ...
                .attr('class', 'mi-chart-svg mi-chart-bar')
                .append('g')
        ...
        if (update) {
            ...
            barGroup.select('.mi-bar-backdrop')
                .transition()
                .duration(500)
                .attr('y', d => this.yChart(this._getLabel(d)))
                .attr('height', this.barHeight());

            barGroup.select('.mi-bar-table-label')
                .transition()
                .duration(500)
            ...
            barGroup.select('.mi-bar-table-value')
                .transition()
                .duration(500)
                .attr('y', d => (this.yChart(this._getLabel(d)) + (this.barHeight() / 2)))
                .text(d => format(d[1]));
        ...
        newGroup.append('rect')
            .attr('class', 'mi-bar-backdrop')
            .attr('x', 0)
            .attr('height', this.barHeight())
            .attr('y', d => this.yChart(that._getLabel(d)))
            .attr('width', this.width() - this.state.margin.left - this.state.margin.right)
            .attr('fill', '#EEE');

        newGroup.append('rect')
            .attr('class', 'mi-bar table')
            .attr('x', 0)
            .attr('height', this.barHeight())
            .attr('y', d => this.yChart(that._getLabel(d)))
            .attr('width', 0)
        ...
        newGroup.append('text')
            .attr('class', 'mi-bar-table-label')
            .attr('y', d => (this.yChart(this._getLabel(d)) + (this.barHeight() / 2)))
            .attr('dy', '.35em')
            .attr('text-anchor', 'start')
            .text(d => {
                if (this.state.labels.maxLength) {
                    return truncate(that._getLabel(d), {
                        length: that.state.labels.maxLength,
                        omission: '...'
                    });
                }
                return this._getLabel(d);
